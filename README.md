...
> El segundo circuito es conocido como integrador, debido a que la señal de salida en verdad es la itegral de la función de entrada, esto se lo comprueba al analizar la integral de una constante, que es la señal de entrada solo que esta cambia en un cierto periodo, vemos que representa una recta que tendrá como pendiente el pico de la señal de entrada, es decir 5 voltios, claro que también cambiara periodicamente como lo hace la señal de entrada ademas de ser forma inversa, ya que cuando la señal de entrada tiene valores positivos, la de salida tiene una pendiente negativa, cuando deberia ser creciente en un caso no invertido.
